    ...
    , pom_break_(Gtk::Adjustment::create(5, 1, 60, 1, 5))
{
    ...
    auto image_filter=Gtk::FileFilter::create(); image_filter->set_name("Pictures"); image_filter->add_pixbuf_formats(); background_image_.add_filter(image_filter);background_light_image_.add_filter(image_filter);background_dark_image_.add_filter(image_filter);
    if(auto path=app_.prefs().background_image_path();path&&!path->empty())background_image_.set_filename(*path);
    if(auto path=app_.prefs().background_light_image_path();path&&!path->empty())background_light_image_.set_filename(*path);
    if(auto path=app_.prefs().background_dark_image_path();path&&!path->empty())background_dark_image_.set_filename(*path);
    auto picture_row=[appear_page](char const* label,Gtk::FileChooserButton& chooser,Gtk::Button& clear){auto* box=Gtk::manage(new Gtk::Box(Gtk::ORIENTATION_HORIZONTAL,8));auto* text=Gtk::manage(new Gtk::Label(label));text->set_width_chars(16);text->set_halign(Gtk::ALIGN_START);box->pack_start(*text,false,false);box->pack_start(chooser,true,true);box->pack_start(clear,false,false);appear_page->pack_start(*box,false,false);};
    picture_row("Both appearances",background_image_,background_image_clear_);picture_row("Light appearance",background_light_image_,background_light_image_clear_);picture_row("Dark appearance",background_dark_image_,background_dark_image_clear_);
    background_image_clear_.signal_clicked().connect([this]{background_image_.unselect_all();});
    background_light_image_clear_.signal_clicked().connect([this]{background_light_image_.unselect_all();});
    background_dark_image_clear_.signal_clicked().connect([this]{background_dark_image_.unselect_all();});
    auto* appearance_help=Gtk::manage(new Gtk::Label("Light and Dark override Both. When the app theme follows the system, the background service reads the current desktop color scheme before rendering."));appearance_help->set_line_wrap(true);appearance_help->set_halign(Gtk::ALIGN_START);appear_page->pack_start(*appearance_help,false,false);
    ...
    auto* save_background=Gtk::manage(new Gtk::Button("Save and update background"));save_background->signal_clicked().connect([this]{std::string command=background_command_.get_text();if(!command.empty()){std::vector<std::string> args;std::string error;if(!expand_background_command(command,"/tmp/background test.png",args,error)){background_status_.set_text(error);return;}}(void)app_.prefs().set_background_command(command);(void)app_.prefs().set_background_image_path(background_image_.get_filename());(void)app_.prefs().set_background_light_image_path(background_light_image_.get_filename());(void)app_.prefs().set_background_dark_image_path(background_dark_image_.get_filename());(void)app_.prefs().set_background_image_fit(background_image_fit_.get_active_id());(void)app_.prefs().set_background_glass_opacity(std::to_string(background_opacity_.get_value()/100.0));sync_.refresh_background();background_status_.set_text(command.empty()?"Automatic background updates disabled":"Background update requested");});appear_page->pack_start(*save_background,false,false);
